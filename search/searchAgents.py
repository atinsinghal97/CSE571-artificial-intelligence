...
def cornersHeuristic(state, problem):
    ...
    # Heuristic- Manhattan distance to a corner that is at max distance

    # I tried implementing Euclidean distance and using it for Heuristic but that led to expanding 1241 nodes
    # which in turn resulted in me getting 2 credits instead of 3
    # Then I remembered that Professor mentioned in class that using Manhattan distance is better than Euclidean for
    # Pacman search while teaching about search heuristics
    ...
    # print state[0], state[0][0], state[0][1], farCorner, farCorner[0], farCorner[1]
    # distanceFarCorner = ((farCorner[0] - state[0][0]) ** 2 + (farCorner[1] - state[0][1]) ** 2) ** 0.5 #Calculating Euclidean distance
    distanceFarCorner = util.manhattanDistance(farCorner, state[0])

    for corner in listOfCorners:
        # if distanceFarCorner < (((corner[0] - state[0][0]) ** 2 + (corner[1] - state[0][1]) ** 2) ** 0.5):
        #     farCorner = corner
        #     distanceFarCorner = ((farCorner[0] - state[0][0]) ** 2 + (farCorner[1] - state[0][1]) ** 2) ** 0.5
