...
class PlanRouteProblem(search.Problem):
    ...
    def actions(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # representation of a state is a triple representing the x, y location and heading, of the agent
        # 0, 1, 2, or 3, representing North, West, South, and East
        # checking if direction-location pair exists in list of allowed moves
        
        if (state[2] == 0 and (state[0], state[1] + 1) in self.allowed) or (
                state[2] == 1 and (state[0] - 1, state[1]) in self.allowed) or (
                state[2] == 2 and (state[0], state[1] - 1) in self.allowed) or (
                state[2] == 3 and (state[0] + 1, state[1]) in self.allowed):
            return ['Forward', 'TurnRight', 'TurnLeft']
        return ['TurnRight', 'TurnLeft'] # Can't go Forward when at terminal squares
    ...
    def result(self, state, action):
        ...
        if state[2]==0: #North
            if action=='Forward':
                return (state[0], state[1]+1, state[2]) #Moves North
            elif action=='TurnRight':
                return (state[0], state[1], 3) #Turns East
            elif action=='TurnLeft':
                return (state[0], state[1], 1) #Turns West
            elif (action=='Grab' or action=='Shoot' or action=='Climb' or action=='Wait'):
                return state

        elif state[2]==1: #West
            if action=='Forward':
                return (state[0]-1, state[1], state[2]) #Moves West
            elif action=='TurnRight':
                return (state[0], state[1], 0) #Turns North
            elif action=='TurnLeft':
                return (state[0], state[1], 2) #Turns South
            elif (action=='Grab' or action=='Shoot' or action=='Climb' or action=='Wait'):
                return state

        elif state[2]==2: #South
            if action=='Forward':
                return (state[0], state[1]-1, state[2]) #Moves South
            elif action=='TurnRight':
                return (state[0], state[1], 1) #Turns West
            elif action=='TurnLeft':
                return (state[0], state[1], 3) #Turns East
            ...
            if action=='Forward':
                return (state[0]+1, state[1], state[2]) #Moves East
            elif action=='TurnRight':
                return (state[0], state[1], 2) #Turns South
            elif action=='TurnLeft':
                return (state[0], state[1], 0) #Turns North
...
class PlanShotProblem(search.Problem):
    ...
    def h(self,node):
        ...
        for x in self.goals: #Getting a list of Wumpus Locations
            for y in self.allowed: #Getting a list of locations we're allowed to visit
                if (x[0]==y[0]) or (x[1]==y[1]):
                    list.append(y)

        return min([manhattan_distance_with_heading(node.state, goal) for goal in x])
    ...
    def goal_test(self, state):
        ...
        if state in self.goals:
            return False #If we need to find wupmus to shoot it and are at wumpus's location, we would be dead

        for x in self.goals:
            if x[0]==state[0]:
                if (x[1]<state[1] and state[2]==2) or (x[1]>state[1] and state[2]==0):
                    return True
            elif x[1]==state[1]:
                if (x[0]>state[0] and state[2]==3) or (x[0]<state[0] and state[2]==1):
                    return True
        return False
