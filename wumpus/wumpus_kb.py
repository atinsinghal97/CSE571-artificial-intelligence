        ...
        for y in range(ymin, ymax+1):
            ...
            for i in range(xmin, xmax+1):
                for j in range(ymin, ymax+1):
                    if (x!=i) or (y!=j):
                        partAxiom.append("~"+wumpus_str(i,j))
            axiom+=" & ".join(partAxiom)
            axiom+="))"
            axioms.append(axiom)
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    placesWhereAgentCantBe=[]

    for x in range(xmin, xmax+1):
        for y in range (ymin, ymax+1):
            if (x!=xi) or (y!=yi):
                placesWhereAgentCantBe.append(state_loc_str(x, y, t))

    axiom_str+="|".join(placesWhereAgentCantBe)
    axiom_str+=")"
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    if heading=='north':
        ...
        axiom_str = '{1} & ~{0} & ~{2} & ~{3}'.format(state_heading_north_str(t), state_heading_east_str(t),
                                                      state_heading_south_str(t), state_heading_west_str(t))
    elif heading=='south':
        axiom_str = '{2} & ~{0} & ~{1} & ~{3}'.format(state_heading_north_str(t), state_heading_east_str(t),
                                                      state_heading_south_str(t), state_heading_west_str(t))
    elif heading=='west':
        axiom_str = '{3} & ~{0} & ~{1} & ~{2}'.format(state_heading_north_str(t), state_heading_east_str(t),
                                                      state_heading_south_str(t), state_heading_west_str(t))
    # print (axiom_str)
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    list = ['({0} & (~{1} | {2} | {3} | {4} | {5} | {6}))'.format(state_loc_str(x, y, t), action_forward_str(t),
                                                                     percept_bump_str(t + 1),
                                                                     action_grab_str(t), action_shoot_str(t),
                                                                     action_turn_left_str(t), action_turn_right_str(t))]

    for ((xi , yi), action) in [(((x - 1), y), 'E'), ((x, (y - 1)), 'N'), (((x + 1), y), 'W'), ((x, (y + 1)), 'S')]:
        if ((xi >= xmin) and (xi <= xmax) and (yi >= ymin) and (yi <= ymax)):
            if action == 'N':
                list.append('({0} & ({1} & {2}))'.format(state_loc_str(x, y - 1, t), state_heading_north_str(t),
                                                            action_forward_str(t)))
            elif action == 'S':
                list.append('({0} & ({1} & {2}))'.format(state_loc_str(x, y + 1, t), state_heading_south_str(t),
                                                            action_forward_str(t)))
            elif action == 'E':
                list.append('({0} & ({1} & {2}))'.format(state_loc_str(x - 1, y, t), state_heading_east_str(t),
                                                            action_forward_str(t)))
            elif action == 'W':
                list.append('({0} & ({1} & {2}))'.format(state_loc_str(x + 1, y, t), state_heading_west_str(t),
                                                            action_forward_str(t)))

    attplusone = state_loc_str(x, y, t + 1)
    att = ' | '.join(list)
    axiom_str = '{0} <=> ({1})'.format(attplusone, att)
    # print (axiom_str)
...
def axiom_generator_heading_north_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_north_str(t + 1) + ' <=> ((' + state_heading_north_str(t) + ' & (' + action_forward_str(
        t) + '|' + action_shoot_str(t) + '|' + action_grab_str(t) + '|' + action_wait_str(t) + '|' + percept_bump_str(
        t + 1) + ')) | (' + state_heading_east_str(t) + ' & ' + action_turn_left_str(
        t) + ') | (' + state_heading_west_str(
        t) + ' & ' + action_turn_right_str(t) + '))'
    # print (axiom_str)
...
def axiom_generator_heading_east_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_east_str(t + 1) + ' <=> ((' + state_heading_east_str(t) + ' & (' + action_forward_str(
        t) + '|' + action_shoot_str(t) + '|' + action_grab_str(t) + '|' + action_wait_str(t) + '|' + percept_bump_str(
        t + 1) + ')) | (' + state_heading_south_str(t) + ' & ' + action_turn_left_str(
        t) + ') | (' + state_heading_north_str(
        t) + ' & ' + action_turn_right_str(t) + '))'
    # print (axiom_str)
...
def axiom_generator_heading_south_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_south_str(t + 1) + ' <=> ((' + state_heading_south_str(t) + ' & (' + action_forward_str(
        t) + '|' + action_shoot_str(t) + '|' + action_grab_str(t) + '|' + action_wait_str(t) + '|' + percept_bump_str(
        t + 1) + ')) | (' + state_heading_west_str(t) + ' & ' + action_turn_left_str(
        t) + ') | (' + state_heading_east_str(
        t) + ' & ' + action_turn_right_str(t) + '))'
    # print (axiom_str)
...
def axiom_generator_heading_west_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = state_heading_west_str(t + 1) + ' <=> ((' + state_heading_west_str(t) + ' & (' + action_forward_str(
        t) + '|' + action_shoot_str(t) + '|' + action_grab_str(t) + '|' + action_wait_str(t) + '|' + percept_bump_str(
        t + 1) + ')) | (' + state_heading_north_str(t) + ' & ' + action_turn_left_str(
        t) + ') | (' + state_heading_south_str(
        t) + ' & ' + action_turn_right_str(t) + '))'
    # print (axiom_str)
...
def axiom_generator_heading_only_east(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{2} <=> (~{1} & ~{0} & ~{3})'.format(state_heading_north_str(t), state_heading_south_str(t),
                                                      state_heading_east_str(t), state_heading_west_str(t))
    # print(axiom_str)
...
def axiom_generator_heading_only_south(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{1} <=> (~{0} & ~{2} & ~{3})'.format(state_heading_north_str(t), state_heading_south_str(t),
                                                      state_heading_east_str(t), state_heading_west_str(t))
    # print(axiom_str)
...
def axiom_generator_heading_only_west(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{3} <=> (~{1} & ~{2} & ~{0})'.format(state_heading_north_str(t), state_heading_south_str(t),
                                                      state_heading_east_str(t), state_heading_west_str(t))
    # print(axiom_str)
...
def axiom_generator_only_one_action_axioms(t):
    ...
    list = [action_forward_str(t), action_grab_str(t), action_shoot_str(t), action_climb_str(t),
            action_turn_left_str(t), action_turn_right_str(t), action_wait_str(t)]
    axioms=[]
    axioms.append('({0} <=> (~{1} & ~{2} & ~{3} & ~{4} & ~{5} & ~{6}))'.format(*list))
    axioms.append('({1} <=> (~{0} & ~{2} & ~{3} & ~{4} & ~{5} & ~{6}))'.format(*list))
    axioms.append('({2} <=> (~{1} & ~{0} & ~{3} & ~{4} & ~{5} & ~{6}))'.format(*list))
    axioms.append('({3} <=> (~{1} & ~{2} & ~{0} & ~{4} & ~{5} & ~{6}))'.format(*list))
    axioms.append('({4} <=> (~{1} & ~{2} & ~{3} & ~{0} & ~{5} & ~{6}))'.format(*list))
    axioms.append('({5} <=> (~{1} & ~{2} & ~{3} & ~{4} & ~{0} & ~{6}))'.format(*list))
    axioms.append('({6} <=> (~{1} & ~{2} & ~{3} & ~{4} & ~{5} & ~{0}))'.format(*list))
    axiom_str='&'.join(axioms)
    # print(axiom_str)
